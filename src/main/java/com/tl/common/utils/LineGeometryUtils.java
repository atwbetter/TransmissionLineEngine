...
public class LineGeometryUtils {
     ...
     * [lon,lat,z,lon,lat,z]
     */
    public static List<LinePoint> parseLine(String line) {
        List<LinePoint> result = new ArrayList<>();
        line = line.replace("[", "").replace("]", "");
        String[] arr = line.split(",");
        for (int i = 0; i < arr.length; i += 3) {
            double lon = Double.parseDouble(arr[i].trim());
            double lat = Double.parseDouble(arr[i + 1].trim());
            double elev = Double.parseDouble(arr[i + 2].trim());
            result.add(new LinePoint(lon, lat, elev));
        }
        return result;
    }
     ...
     * 单位:米
     */
    public static double distance(LinePoint p1, LinePoint p2) {
     ...
     * 计算线路总长度
     */
    public static double lineLength(List<LinePoint> points) {
        double length = 0;
     ...
     * 0-360°
     */
    public static double azimuth(LinePoint p1, LinePoint p2) {
        double lat1 = Math.toRadians(p1.lat);
        double lat2 = Math.toRadians(p2.lat);
        double dLon = Math.toRadians(p2.lon - p1.lon);
     ...
     * 1 终点
     */
    public static LinePoint interpolate(LinePoint p1, LinePoint p2, double ratio) {
        double lon = p1.lon + (p2.lon - p1.lon) * ratio;
        double lat = p1.lat + (p2.lat - p1.lat) * ratio;
        double elev = p1.elev + (p2.elev - p1.elev) * ratio;
        return new LinePoint(lon, lat, elev);
    }
     ...
     * 采样间隔 米
     */
    public static List<LinePoint> sampleLine(List<LinePoint> line, double sampleDistance) {
        List<LinePoint> result = new ArrayList<>();
    ...
    public static void main(String[] args) {
        String line = "[112.653303,26.006398,370.5," + "112.655047,26.00851,333.1," + "112.656158,26.010697,397.1," + "112.659456,26.014043,507.2]";
